...
import { Check, Calendar, MapPin, ArrowLeft, Info, Zap, ChevronDown, CheckCircle2 } from "lucide-react";
...
export default function BroomITPage() {
    ...
    // Form Submission to Dedicated Isolated Backend API (/api/broomit/bookings)
    const handleSubmitBooking = async () => {
        if (!user) {
            toast.error("Please log in to complete your booking");
            router.push(`/login?redirect=/broomit`);
            return;
        }
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/broomit/bookings`, {
                method: "POST",
                headers: {
            ...
            const data = await res.json();
            if (!res.ok) throw new Error(data.message || "Failed to submit 15-min booking");

            setBookingSuccess(data.bookingNumber || data._id || `BRM15-${Math.floor(100000 + Math.random() * 900000)}`);
            toast.success("15-Min Maid Request Dispatched!");
            window.scrollTo({ top: 0, behavior: 'smooth' });
        } catch (err) {
            console.error(err);
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div>
                        ...
                        <p className="text-slate-500 text-sm md:text-base font-medium mt-1">
                            Fast, reliable, and trained household support at your doorstep.
                        </p>
                    </div>
                </div>
                ...
                {bookingSuccess ? (
                    <div className="bg-white rounded-3xl p-8 md:p-12 shadow-xl border border-slate-100 text-center max-w-2xl mx-auto animate-in fade-in zoom-in duration-500">
                        ...
                        <p className="text-slate-500 mb-6">
                            Your <strong className="text-slate-800">Help in 15 Mins</strong> request has been dispatched. Our team will assign the nearest verified worker immediately.
                        </p>

                        <div className="bg-amber-50 border border-amber-200 p-5 rounded-2xl mb-8">
                            <p className="text-xs uppercase font-extrabold tracking-wider text-amber-800 mb-1">BroomIT Booking Reference</p>
                            <p className="text-2xl font-mono font-bold text-slate-900">#{bookingSuccess.toString().slice(-6).toUpperCase()}</p>
                            <p className="text-xs text-amber-700 mt-2 font-medium">Estimated Worker ETA: ~15 Minutes</p>
                        </div>

                        <div className="flex flex-col sm:flex-row gap-4">
                            <button
                                onClick={() => router.push("/dashboard/broomit")}
                                className="flex-1 bg-slate-900 hover:bg-slate-800 text-white font-bold py-3.5 px-6 rounded-xl transition-all shadow-md"
                            >
                                Track in BroomIT Dashboard
                            </button>
                            <button
                                onClick={() => {
                                ...
                                className="flex-1 bg-amber-400 hover:bg-amber-500 text-slate-950 font-bold py-3.5 px-6 rounded-xl transition-all shadow-md"
                            >
                    ...
                    <>
                        {/* PHASE 1: TASK & REGION SELECTION (IMAGE 2 UI) */}
                        {step === 1 && (
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                                ...
                                <div className="lg:col-span-7 bg-white rounded-3xl p-6 md:p-8 shadow-xl border border-slate-100 relative">
                                    ...
                                    <div className="mb-6">
                                        <div className="relative">
                                            ...
                                            <select
                                                ...
                                                className="w-full pl-12 pr-10 py-3.5 bg-white border border-slate-200 rounded-2xl font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-400/50 appearance-none shadow-sm cursor-pointer"
                                            >
                                    ...
                                    <div className="flex items-center justify-between border-t border-slate-100 pt-6">
                                        <div className="text-xs font-mono font-bold text-slate-500 flex items-center gap-1.5">
                                            <Zap size={14} className="text-amber-500 fill-amber-400 animate-pulse" />
                                            <span>~ETA: 15 minutes</span>
                                        </div>

                                        <button
                                            onClick={handleProceedToDetails}
                                            className="px-8 py-3.5 bg-amber-400 hover:bg-amber-500 text-slate-950 rounded-full font-bold text-sm md:text-base shadow-lg shadow-amber-200 transition-all hover:scale-[1.02]"
                                        >
                                            Proceed
                                        </button>
                                    </div>
                                ...
                                {/* Right Side Visual Gallery Showcase */}
                                <div className="lg:col-span-5 bg-gradient-to-br from-amber-400 via-amber-300 to-yellow-400 p-1.5 rounded-3xl shadow-xl">
                                    <div className="bg-slate-900 rounded-[22px] overflow-hidden grid grid-cols-2 gap-1.5 p-1.5 min-h-[380px]">
                                        ...
                                        <div className="relative rounded-xl overflow-hidden group min-h-[180px] col-span-2">
                                            ...
                                            <div className="absolute inset-0 bg-gradient-to-t from-slate-950/90 via-slate-950/20 to-transparent flex flex-col justify-end p-5">
                                                <span className="text-amber-400 text-xs font-mono uppercase font-bold tracking-widest">Instant 15-Min Arrival</span>
                        ...
                        {/* PHASE 2: BOOKING FORM & EXACT PRICING BREAKDOWN (IMAGE 3 UI) */}
                        {step === 2 && (
                            <div className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden max-w-4xl mx-auto">
                                ...
                                <div className="p-6 md:p-8 space-y-6">
                                    ...
                                    {/* Address Field */}
                                    <div className="border border-slate-200 rounded-2xl p-4 flex items-center justify-between gap-4 bg-white hover:border-slate-300 transition-colors">
                                        <div className="flex items-center gap-3 flex-1">
                                            <MapPin size={22} className="text-amber-500 flex-shrink-0" />
                                            <input
                                                type="text"
                                                value={address}
                                        ...
                                        <button
                                            ...
                                            className="text-xs md:text-sm font-bold text-amber-600 hover:text-amber-700 flex-shrink-0"
                                        >
                                            + Add Address
                                        </button>
                                    </div>

                                    {/* Phone Number Field matching standard booking format */}
                                    <div className="border border-slate-200 rounded-2xl p-4 bg-white">
                                        ...
                                        <div className="flex h-12">
                                            ...
                                            <input
                                                ...
                                                className="w-full px-4 bg-slate-50 border border-slate-200 rounded-r-xl focus:outline-none focus:border-amber-400 font-medium text-slate-700"
                                            />
                                        </div>
                                    </div>

                                    {/* Tiered Reference Field: No. of Hours */}
                                            ...
                                            {HOUR_OPTIONS.map((opt) => {
                                                ...
                                                return (
                                                    <div key={opt.hours} className="relative">
                                                        {opt.bestseller && (
                                                            <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-gradient-to-r from-pink-500 to-purple-600 text-white text-[10px] font-black uppercase px-2 py-0.5 rounded-full shadow-sm z-10 whitespace-nowrap">
                                                                Bestseller
                                                            </div>
                                                        )}
                                                        <button
                                                            ...
                                                            className={cn(
                                                                ...
                                                                isSelected
                                                                    ...
                                                                    : "bg-amber-50/50 border-slate-200 text-slate-700 hover:bg-amber-50"
                                                            )}
                                                        >
                                                            {opt.label}
                                                        </button>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                preferences.petAtHome ? "border-amber-400 bg-amber-50 text-amber-900" : "border-slate-200 bg-white text-slate-700"
                                            )}
                                        >
                                            ...
                                            <span className={cn(
                                                ...
                                                preferences.petAtHome ? "border-amber-500 bg-amber-500 text-white" : "border-slate-300"
                                            )}>
                                        ...
                                        <button
                                            ...
                                            className={cn(
                                                "p-3.5 rounded-2xl border flex items-center justify-between text-xs md:text-sm font-bold transition-all",
                                                preferences.avoidCalling ? "border-amber-400 bg-amber-50 text-amber-900" : "border-slate-200 bg-white text-slate-700"
                                            )}
                                        >
                                            <span className="flex items-center gap-2">
                                                <span>🔕</span>
                                            ...
                                            <span className={cn(
                                                "w-5 h-5 rounded-full border-2 flex items-center justify-center",
                                                preferences.avoidCalling ? "border-amber-500 bg-amber-500 text-white" : "border-slate-300"
                                            )}>
                                        ...
                                        <button
                                            ...
                                            className={cn(
                                                "p-3.5 rounded-2xl border flex items-center justify-between text-xs md:text-sm font-bold transition-all",
                                                preferences.femaleWorkerPreferred ? "border-amber-400 bg-amber-50 text-amber-900" : "border-slate-200 bg-white text-slate-700"
                                            )}
                                        >
                                            <span className="flex items-center gap-2">
                                                <span>👩</span>
                                            ...
                                            <span className={cn(
                                                "w-5 h-5 rounded-full border-2 flex items-center justify-center",
                                                preferences.femaleWorkerPreferred ? "border-amber-500 bg-amber-500 text-white" : "border-slate-300"
                                            )}>
                                    ...
                                    <div className="border border-slate-200 rounded-2xl p-2 bg-white flex items-center justify-between gap-3">
                                        ...
                                        <button
                                            ...
                                            className="px-6 py-2.5 bg-amber-400 hover:bg-amber-500 text-slate-950 font-bold text-xs md:text-sm rounded-xl transition-all flex-shrink-0 shadow-sm"
                                        >
                                            Apply
                                        </button>
                                    </div>

                                    {/* PAYMENT DETAILS SECTION MATCHING MAIN BOOKING FLOW PATTERN */}
                                    <div className="bg-slate-50 border border-slate-200 p-6 rounded-2xl">
                                        ...
                                        <div className="flex flex-col md:flex-row gap-6">
                                            <div className="flex-1">
                                                ...
                                                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-4 flex justify-center">
                                                    <img src="/images/qr.jpeg" alt="Payment QR Code" className="rounded-lg object-contain w-[180px] h-[180px]" />
                                                </div>
                                                <div className="space-y-1 text-xs bg-white p-4 rounded-xl border border-slate-200 shadow-sm text-slate-700">
                                    ...
                                    {/* EXACT PRICING BREAKDOWN CARD MATCHING IMAGE 3 */}
                                    <div className="bg-slate-50 border border-slate-200/90 rounded-2xl p-5 space-y-3 font-medium text-slate-600 text-sm">
                                        ...
                                        <div className="border-t border-slate-300 pt-3 flex items-center justify-between text-slate-900 font-extrabold text-base md:text-lg">
                                            ...
                                            <span className="text-xl md:text-2xl font-black text-slate-950">₹{amountToBePaid.toFixed(2)}</span>
                                        </div>
                                    </div>
                                    ...
                                    <button
                                        ...
                                        className="w-full py-4 bg-amber-400 hover:bg-amber-500 text-slate-950 font-extrabold text-base md:text-lg rounded-2xl transition-all shadow-lg shadow-amber-200 flex items-center justify-center gap-2"
                                    >
