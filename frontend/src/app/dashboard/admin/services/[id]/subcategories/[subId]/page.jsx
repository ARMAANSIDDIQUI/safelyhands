...
export default function EditSubCategoryPage() {
    ...
    const fetchSubCategory = async () => {
        try {
            // Need an endpoint to get single subcategory. 
            // I haven't created GET /api/subcategories/:id yet.
            // But I can fetch via service? No, usually direct ID.
            // I'll assume I'll add GET /api/subcategories/:id
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/subcategories/${subId}`);
            if (res.ok) {
    ...
    // --- Reuse Logic (should have been a component, but copying for speed per user rules) ---
    ...
    const addQuestionStep = () => {
        setFormData({ ...formData, questions: [...formData.questions, { stepTitle: "Details", fields: [] }] });
    };
    ...
    const addFieldToStep = (stepIndex) => {
        const newQuestions = [...formData.questions];
        newQuestions[stepIndex].fields.push({ name: "", label: "", type: "radio", options: [{ label: "", value: "", priceChange: 0 }], required: true });
        setFormData({ ...formData, questions: newQuestions });
    };
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto">
            <div className="flex items-center gap-4 mb-6">
                ...
                <h1 className="text-2xl font-bold text-slate-800">Edit SubCategory</h1>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="sticky bottom-4 bg-white p-4 rounded-xl border border-slate-200 shadow-xl flex justify-between items-center">
                    ...
                    >
                        {submitting ? "Saving..." : "Update SubCategory"}
                    </button>
                </div>
            </form>
        </div>
