...
const STATUS_CONFIG = {
    pending_dispatch: { label: "Pending Dispatch", color: "bg-blue-100 text-blue-900 border-blue-300", step: 1 },
    worker_assigned: { label: "Worker Assigned", color: "bg-sky-100 text-sky-900 border-sky-300", step: 2 },
    dispatched: { label: "En Route (~15 Mins)", color: "bg-indigo-100 text-indigo-900 border-indigo-300", step: 3 },
    arrived: { label: "Worker Arrived", color: "bg-teal-100 text-teal-900 border-teal-300", step: 4 },
    in_progress: { label: "In Progress", color: "bg-blue-100 text-blue-900 border-blue-300", step: 5 },
...
export default function UserSafeITDashboard() {
    const { user } = useAuth();
    const [bookings, setBookings] = useState([]);
    const [loading, setLoading] = useState(true);

    const fetchMySafeitBookings = async () => {
        if (!user?.token) return;
        try {
            setLoading(true);
            let res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/safeit/my-bookings`, {
                headers: {
                    Authorization: `Bearer ${user.token}`
                }
            });
            if (!res.ok) {
                res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/broomit/my-bookings`, {
                    headers: {
                        Authorization: `Bearer ${user.token}`
                    }
                });
            ...
            console.error("Failed to fetch SafeIt bookings:", err);
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        fetchMySafeitBookings();
    }, [user]);

    return (
        <div className="p-6 md:p-10 max-w-6xl mx-auto space-y-8 font-sans">
            {/* Header banner - Blue Theme Matched */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-blue-600 via-sky-500 to-indigo-600 p-6 md:p-8 rounded-3xl shadow-lg text-white">
                <div>
                    <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-white/20 backdrop-blur-md text-white rounded-full text-xs font-bold uppercase tracking-wider mb-2">
                        <Zap size={14} className="fill-current text-white" />
                        <span>15-Min SafeIt Tracker</span>
                    </div>
                ...
                <div className="flex items-center gap-3">
                    <button
                        onClick={fetchMySafeitBookings}
                        className="p-3 bg-white/20 hover:bg-white/30 text-white rounded-2xl shadow-xs transition-all backdrop-blur-md"
                    ...
                    <Link
                        href="/safeit"
                        className="px-6 py-3.5 bg-white hover:bg-slate-100 text-blue-900 font-extrabold text-sm rounded-2xl transition-all shadow-md flex items-center gap-2"
                    >
            ...
            {loading ? (
                <div className="space-y-4">
                    {[1, 2].map((n) => (
                        <div key={n} className="h-48 bg-white rounded-3xl animate-pulse shadow-xs border border-slate-100" />
                    ))}
                </div>
            ) : bookings.length === 0 ? (
                <div className="bg-white rounded-3xl p-12 text-center shadow-xs border border-slate-100 max-w-lg mx-auto">
                    <div className="w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-4 text-blue-600">
                        <Zap size={32} />
                    </div>
                    <h3 className="text-xl font-bold text-slate-900 mb-2">No SafeIt Bookings Yet</h3>
                    <p className="text-slate-500 text-sm mb-6">
                        Need instant help with brooming, mopping, dusting, or dish washing? Get a verified worker dispatched in 15 minutes!
                    </p>
                    <Link
                        href="/safeit"
                ...
                <div className="space-y-6">
                    {bookings.map((booking) => {
                        ...
                        return (
                            <div
                                key={booking._id}
                                className="bg-white rounded-3xl p-6 md:p-8 shadow-xs border border-slate-200/80 hover:shadow-md transition-all space-y-6"
                            >
                                {/* Top Bar */}
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
                                    ...
                                    <div className="self-start sm:self-auto flex items-center gap-2">
                                        <span className={cn(
                                            "px-4 py-1.5 rounded-full text-xs font-black border uppercase tracking-wider flex items-center gap-1.5 shadow-xs",
                                            statusInfo.color
                                        )}>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="space-y-1.5">
                                            <p className="text-sm font-bold text-slate-800">Duration: <span className="text-blue-600">{booking.hours} Hours</span></p>
                                            <div className="flex flex-wrap gap-1.5">
                                                {booking.tasks.map((task, i) => (
                                                    <span key={i} className="text-xs font-semibold bg-blue-50 text-blue-950 border border-blue-200 px-2.5 py-1 rounded-lg">
                                                        {task}
                                                    </span>
                                                ))}
                                            </div>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        {booking.assignedWorker ? (
                                            ...
                                            <div className="bg-blue-50 border border-dashed border-blue-300 p-3 rounded-2xl text-xs text-blue-900 font-semibold flex items-center gap-2">
