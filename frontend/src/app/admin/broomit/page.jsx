...
const STATUS_CONFIG = {
    pending_dispatch: { label: "Pending Dispatch", color: "bg-amber-100 text-amber-900 border-amber-300" },
    worker_assigned: { label: "Worker Assigned", color: "bg-blue-100 text-blue-900 border-blue-300" },
    dispatched: { label: "En Route (~15m)", color: "bg-purple-100 text-purple-900 border-purple-300" },
    arrived: { label: "Worker Arrived", color: "bg-teal-100 text-teal-900 border-teal-300" },
    in_progress: { label: "In Progress", color: "bg-sky-100 text-sky-900 border-sky-300" },
...
export default function AdminBroomITManagement() {
    const { user } = useAuth();
    ...
    const fetchAdminData = async () => {
        ...
        try {
            ...
            // Fetch BroomIT bookings
            const bookingsRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/broomit/admin/bookings`, {
            ...
            console.error("Failed to load admin BroomIT data:", err);
    ...
    const handleAssignWorker = async (bookingId, workerId) => {
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/broomit/admin/bookings/${bookingId}/assign-worker`, {
                method: "PATCH",
                headers: {
    ...
    const handleUpdateStatus = async (bookingId, newStatus) => {
        try {
            setUpdatingId(bookingId);
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/broomit/admin/bookings/${bookingId}/status`, {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${user.token}`
    ...
    return (
        <div className="p-6 md:p-10 max-w-7xl mx-auto space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 text-white p-6 md:p-8 rounded-3xl shadow-xl">
                <div>
                    <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-400 text-slate-950 rounded-full text-xs font-black uppercase tracking-wider mb-2">
                        <Zap size={14} className="fill-slate-950" />
                        <span>Admin Dispatch Panel</span>
                    </div>
                    <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight">
                        BroomIT 15-Min Instant Queue
                    </h1>
                ...
                <button
                    ...
                    className="self-start md:self-auto px-5 py-2.5 bg-amber-400 hover:bg-amber-500 text-slate-950 font-bold text-sm rounded-xl transition-all shadow-md flex items-center gap-2"
                >
                    <RefreshCw size={16} />
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="bg-amber-50 p-5 rounded-2xl border border-amber-200 shadow-sm">
                    <span className="text-xs font-extrabold uppercase text-amber-800 tracking-wider">Pending Dispatch</span>
                    <p className="text-2xl font-black text-amber-900 mt-1">{pendingDispatchCount}</p>
                </div>

                <div className="bg-sky-50 p-5 rounded-2xl border border-sky-200 shadow-sm">
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-4 justify-between items-center">
                <div className="relative w-full md:w-80">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:border-amber-400"
                    />
                </div>
                                    ...
                                    return (
                                        <tr key={b._id} className="hover:bg-slate-50/80 transition-colors">
                                            ...
                                            <td className="p-4 align-top">
                                                ...
                                                <span className="inline-block mt-1 text-[10px] font-bold uppercase bg-amber-100 text-amber-900 px-2 py-0.5 rounded">
                                                    {b.bookingType}
                                                </span>
                                            </td>
                                            ...
                                            <td className="p-4 align-top max-w-xs">
                                                ...
                                                <p className="text-xs text-amber-700 font-semibold">📞 +91 {b.phone}</p>
                                            ...
                                            <td className="p-4 align-top min-w-[200px]">
                                                <select
                                                    ...
                                                    className="w-full p-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-800 focus:outline-none focus:border-amber-400"
                                                >
                                                ...
                                                <div className="flex flex-col gap-1 items-end">
                                                    {b.status === 'pending_dispatch' && (
                                                        <button
                                                            ...
                                                            className="px-3 py-1 bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs rounded-lg transition-all"
                                                        >
                                                            Dispatch ~15m ⚡
                                                        </button>
                                                    )}
                                                    ...
                                                    {['arrived', 'dispatched', 'worker_assigned'].includes(b.status) && (
                                                        <button
                                                            ...
                                                            className="px-3 py-1 bg-sky-600 hover:bg-sky-700 text-white font-bold text-xs rounded-lg transition-all"
                                                        >
                                                            Start Session
                                                        </button>
                                                    )}
